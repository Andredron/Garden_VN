...
style slider:
    ...
    right_bar Frame("gui/button/radio_foreground.png",6,5) #"gui/slider/horizontal_[prefix_]bar.png", gui.slider_borders, tile=gui.slider_tile)
    thumb "gui/slider/horizontal_[prefix_]thumb.png"
    left_bar Frame("gui/button/radio_selected_foreground.png", 10, 10)
    left_gutter 7
    right_gutter 7
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            yalign 0.967
            button:
                add "gui/button/quick/rol.png"
                action Rollback()
            button:
                add "gui/button/quick/his.png"
                action ShowMenu('history')
            button:
                add "gui/button/quick/ski.png"
                action Skip() alternate Skip(fast=True, confirm=True)
            button:
                add "gui/button/quick/sav.png"
                action ShowMenu('save')
            button:
                add "gui/button/quick/loa.png"
                action ShowMenu('load')
            button:
                add "gui/button/quick/pre.png"
...
style quick_button:
    idle_background Solid ("#0000")
    hover_background Solid ("#0000")
    activate_sound "audio/sound/Cursor.wav"
...
screen navigation():
    ...
    vbox:
        ...
        pos  If(renpy.get_screen("main_menu"), (825, 0.1), (gui.navigation_xpos, 0.025))

        spacing If(renpy.get_screen("main_menu"), gui.navigation_spacing, 12)

        if main_menu:

            textbutton _("Начать") at xz action Start()

        else:

            textbutton _("Сохранить") at xz action ShowMenu("save")

        textbutton _("Загрузить") at xz action ShowMenu("load")

        textbutton _("Настройки") at xz action ShowMenu("preferences")
        ...
        if _in_replay:
            ...
            textbutton _("Гл. меню") at xz action MainMenu()


        if renpy.variant("pc") and  renpy.get_screen("main_menu"):

            textbutton _("Выход") at xz action Quit(confirm=not main_menu)
...
style navigation_button:
    ...
    idle_background Frame("gui/button/nav_button.png", 20, 20)
    hover_background Frame("gui/button/nav_button.png", 20, 20)
    xysize (580,135)
    activate_sound "audio/sound/Cursor.wav"
...
screen game_menu(title, scroll=None, yinitial=0.0):
    ...
    textbutton _("Вернуться") at xz style "return_button" action Return()
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        $ nom=slot+(int(FileCurrentPage())-1)*4
                        text _("Слот [nom]: ") + FileTime(slot, format=_("{#file_time}%d %b/%H:%M"), empty=_("Пустой слот")):
...
screen confirm(message, yes_action, no_action):
    ...
    add Solid("#a89c9bdd") #"gui/overlay/confirm.png"
